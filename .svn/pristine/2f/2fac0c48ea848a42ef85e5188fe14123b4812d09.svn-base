...
//	}
	...
	public PageList<T> query(T t, PageBounds pageBounds) {
		return getDao().query(t, pageBounds);
	}
	
	public PageList<T> queryPage(T t) {
		return getDao().query(t, ((BaseEntity)t).toPageBounds());
	}
...
//		String id = null;
//		try {
//			id = org.apache.commons.beanutils.BeanUtils.getSimpleProperty(t, "id");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
...
//	}
	 ...
	 * @param str  &lt;p&gt;aaa&lt;br/&gt;&lt;/p&gt;===><p>aaa<br/></p>
	 * 1:调用方法
	 * 2:对象数据不对
	 * @return
	 */
	 ...
	 * @return
	 */
	protected String doEditor2(String str) {
		if (StringUtils.isNotEmpty(str)) {
			System.out.println("sss");
			return StringEscapeUtils.unescapeHtml3(str).replace("\n", "")
//					.replace("\"", "")
	...
	public  T selectByPrimaryKey2(Pk id) {
		return (T) getDao().selectByPrimaryKey(id);
	}
